...
for i in range(len(df)):
  # print(df['t2_1gt'][i])
  if df['t1_gt'][i] + int(df['t2_1gt'][i]) + df['t2_2gt'][i] + df['t3_gt'][i] == 0 or df['t1_gt'][i] + int(df['t2_1gt'][i]) + df['t2_2gt'][i] + df['t3_gt'][i] > 1:
    if df['sample_dataset'][i] == 'vqacpv2':
      if df['t1_gt'][i] + int(df['t2_1gt'][i]) + df['t2_2gt'][i] + df['t3_gt'][i] == 0:
        classification_gt.append(rankings['vqacpv2'][0])
      else:
        curr_ranking = rankings['vqacpv2']
  ...
  image_path.append("train_images/" + df["image_path"][i])
